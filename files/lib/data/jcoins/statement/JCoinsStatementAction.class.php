...
class JCoinsStatementAction extends AbstractDatabaseObjectAction
{
    ...
    {
        // receiver
        if (empty($this->parameters['receiver'])) {
            return ['receiverError' => WCF::getLanguage()->get('wcf.jcoins.transfer.error.receiver.empty')];
        }
        ...
        if (!$this->parameters['moderative']) {
            ...
            if ($toPay > WCF::getUser()->jCoinsAmount) {
                return ['amountError' => WCF::getLanguage()->getDynamicVariable('wcf.jcoins.transfer.error.tooMuch')];
            }
        }
