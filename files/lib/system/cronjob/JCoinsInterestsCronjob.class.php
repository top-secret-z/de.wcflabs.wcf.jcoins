...
class JCoinsInterestsCronjob extends AbstractCronjob {
	...
	public function execute(Cronjob $cronjob) {
		...
		// only if configured
		if (!MODULE_JCOINS || !JCOINS_INTERESTS_ENABLE || !JCOINS_INTERESTS_RATE) return;
		...
		switch (JCOINS_INTERESTS_PERIOD) {
			...
			case 'quarter':
				if ($month < 4) $exec = strtotime('1st April ' . (string)$year . ' midnight');
				elseif ($month < 7) $exec = strtotime('1st July ' . (string)$year . ' midnight');
				elseif ($month < 10) $exec = strtotime('1st October ' . (string)$year . ' midnight');
				else $exec = strtotime('1st January ' . (string)($year + 1) . ' midnight');
				break;
				
			case 'halfyear':
				if ($month > 6) $exec = strtotime('1st January ' . (string)($year + 1) . ' midnight');
				else $exec = strtotime('1st July ' . (string)$year . ' midnight');
				break;
